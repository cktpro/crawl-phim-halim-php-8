...
function ophim_custom_meta_box($post, $metabox)
{
	...
	?>
	<div class="inside">
		<label for="fetch_ophim_id">OPhim ID: </label><input style="width: 100%" name="fetch_ophim_id" type="text"
			id="fetch_ophim_id" value="<?php echo ($_halim_metabox_options['fetch_ophim_id']) ?? ''; ?>">
		<label for="fetch_ophim_update_time">Thời gian cập nhật: </label><input style="width: 100%"
			name="fetch_ophim_update_time" type="text" id="fetch_ophim_update_time"
			value="<?php echo $_halim_metabox_options["fetch_ophim_update_time"] ?? ''; ?>">
	</div>
	<?php
}
...
function ophim_custom_save_metabox($post_id, $post)
{
	...
	if (!empty($fetch_ophim_id)) {
		$_halim_metabox_options["fetch_ophim_id"] = $fetch_ophim_id;
	}
	if (!empty($fetch_ophim_update_time)) {
...
if (file_exists(CRAWL_OPHIM_PATH . 'functions.php')) {
	include_once CRAWL_OPHIM_PATH . 'functions.php';
} else {
	wp_die('Missing functions.php in Crawl Phim plugin!');
}

if (file_exists(CRAWL_OPHIM_PATH . 'crawl_movies.php')) {
	include_once CRAWL_OPHIM_PATH . 'crawl_movies.php';
} else {
	wp_die('Missing crawl_movies.php in Crawl Phim plugin!');
}