...
function crawl_ophim_schedule_select()
{
	...
	$schedule['enable_custom'] = $_POST['select_source'] === 'custom' ? true : false;
...
// Action crawl list phim custom
add_action('wp_ajax_crawl_link_custom', 'crawl_link_custom');
function crawl_link_custom()
{

	$list = $_POST['listmovies'];

	// echo crawl_ophim_custom_list($list); // array
	$result = crawl_ophim_custom_list_kkphim($list, );
...
function crawl_ophim_custom_list_kkphim($list)
{
	...
	try {
		...
		if ($total > 0) { # Trường hợp đã có
			...
			if ($wp_query->have_posts()):
				while ($wp_query->have_posts()):
					...
					if ($_halim_metabox_options["fetch_ophim_update_time"] == $update_time) { // Không có gì cần cập nhật
					...
					$data = create_data_kkphim_custom($source_item, $url, $update_time, $title, $org_title, $thumb, $cat, $showtime);
					...
					// Re-Update Episodes
					$list_episode = get_list_episode_kkphim($source_item, $post->ID);
					$result = array(
						'status' => true,
						'post_id' => $post->ID,
						'data' => $data,
		...
		$list_episode = get_list_episode_kkphim($source_item, $post_id);
		$result = array(
			'code' => 200,
			'status' => true,
			'post_id' => $post_id,
...
function create_data_kkphim_custom($sourcePage, $url, $ophim_update_time, $title, $org_title, $thumb, $cat, $showtime)
{
	...
	$data = array(
		'crawl_filter' => false,
		'fetch_url' => $url,
		'fetch_ophim_id' => $sourcePage["movie"]["_id"],
...
function crawl_ophim_custom_list($list)
{
	$title = $list['movie_name'];
	$thumb = $list['img_src'];
	$cat = $list['list_cate'];
	...
	try {
		...
		if ($total > 0) { # Trường hợp đã có
			...
			if ($wp_query->have_posts()):
				while ($wp_query->have_posts()):
					...
					// Trường hợp phim chưa có -> tạo mới
					// $api_url 			= str_replace('ophim.tv', 'ophim1.com', $url);
					$data = create_data_custom($source_item, $url, $update_time, $title, $org_title, $thumb, $cat, $showtime);
					...
					// Re-Update Episodes
					$list_episode = get_list_episode_nguonc($source_item, $post->ID);
					$result = array(
						'status' => true,
						'post_id' => $post->ID,
						'data' => $data,
		...
		$post_id = add_posts($data);
		$list_episode = get_list_episode_nguonc($source_item, $post_id);
		$result = array(
			'code' => 200,
			'status' => true,
			'post_id' => $post_id,
...
function create_data_custom($sourcePage, $url, $ophim_update_time, $title, $org_title, $thumb, $cat, $showtime)
{
	...
	if ($status == "ongoing") {
		$current = $sourcePage["movie"]["current_episode"];
		preg_match('/\d+/', $current, $matches);
		$number = (int) $matches[0];

		if ($number > 0) {
			$runtime = "Next Tập " . ($number + 1);
		} else {
			$runtime = "";
...
// End action crawl list phim custom
...
function crawl_ophim_movies_handle_nguonc_custom($url, $filterType, $filterCategory, $filterCountry)
{
	...
	try {
		...
		$source_item = json_decode($source_url);
		$id_phim = $source_item->movie->id;
		$ophim_update_time = $source_item->movie->modified;
		if (!$id_phim || !$ophim_update_time) {
		...
		if ($total > 0 && $total < 2) { # Trường hợp đã có
			$args = array(
				'post_type' => 'post',
				'posts_per_page' => 1,
				'meta_query' => array(
...
function add_posts($data)
{
	...
	$formality = ($data['type'] == 'tv_series') ? 'tv_series' : 'single_movies'; // Lấy ID của category
	$post_data = array(
	...
	$keyword_seo = $data['title'] . ' ' . $data['lang'];
	...
	halim_set_yoast_focuskw($post_id, $keyword_seo);
...
function save_image($image_url, $post_id, $posttitle, $suffix, $max_w, $max_h, $is_webp = false, $set_thumb = false)
{
...
function halim_set_yoast_focuskw($post_id, $keyword)
{
	if (empty($post_id))
		return;
	...
	// Update keyword Yoast SEO (focus keyword)
	if (!empty($keyword)) {
		update_post_meta($post_id, '_yoast_wpseo_focuskw', sanitize_text_field($keyword));
		// _yoast_wpseo_primary_category null
		update_post_meta($post_id, '_yoast_wpseo_primary_category', sanitize_text_field(''));
		// _yoast_wpseo_primary_status null
		update_post_meta($post_id, '_yoast_wpseo_primary_status', sanitize_text_field(''));
		// _yoast_wpseo_primary_post_options null
		update_post_meta($post_id, '_yoast_wpseo_primary_post_options', sanitize_text_field(''));
		// _yoast_wpseo_metadesc %%title%% - %%excerpt%%
		update_post_meta($post_id, '_yoast_wpseo_metadesc', sanitize_text_field('%%excerpt%%'));
		update_post_meta($post_id, '_yoast_wpseo_linkdex', sanitize_text_field(90));
		update_post_meta($post_id, '_yoast_wpseo_content_score', sanitize_text_field(90));
	}
	// Reindex post for Yoast SEO
